class Category < ActiveRecord::Base
  ...
  def websites_sorted_by_quality(community_name)
    website_tuple_list = []
    website_list = []
    self.subcategories.each do |subcategory|
      subcategory.websites.each do |website|
  ...
  def websites_sorted_by_newest(community_name)
    website_tuple_list = []
    website_list = []
    self.subcategories.each do |subcategory|
      subcategory.websites.each do |website|
        website_tuple_list << [website, website.created_at]
      end
    end
    website_tuple_list.sort! { |a,b| b[1] <=> a[1] }
    website_tuple_list.each do |website_tuple|
      website_list << [website_tuple[0], website_tuple[0].get_rating(community_name).get_vote_differential]
    end
    return website_list
  end
